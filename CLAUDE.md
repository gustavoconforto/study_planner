...
- **Database**: Drizzle ORM against Neon serverless Postgres. Schema in `src/db/schema.ts`, client in `src/db/db.ts` (`drizzle-orm/neon-http`, reads `DATABASE_URL`). `drizzle.config.ts` outputs migrations to `./drizzle`. Two tables so far: `agendamentoTable` (`agendamentos`, backing `disponibilidade`) and `tarefaTable` (`tasks`, backing `tarefas`), the latter with several pgEnums (`TASK_TYPE`, `TASK_DIFICULTY`, `TASK_STATUS`, `TASK_SUBJECT`) whose value arrays are exported alongside the enum for reuse in form options/types.
...
- **Shared layout pieces**: `src/app/layout/PageHeader/` renders each page's title/subtitle/CTA banner and is used by `src/app/page.tsx`, `disponibilidade/page.tsx`, and `tarefas/page.tsx`. `src/app/layout/Footer/` is used only on the home page (`src/app/page.tsx`). Neither is used by the *global* header/nav — `src/app/layout.tsx` still inlines that markup directly in `RootLayout`, so global-nav edits (e.g. adding a new feature link) go in `layout.tsx`, not in `layout/PageHeader`.
